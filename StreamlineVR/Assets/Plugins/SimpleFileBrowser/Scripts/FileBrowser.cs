...
#pragma warning restore 0414
    ...
    //CUSTOM CODE
    //*************************************************************************
    public static void CustomLoadDialog(OnSuccess onSuccess, OnCancel onCancel,
                       bool folderMode = false, string initialPath = null,
    ...
    public static void ToggleFolderSelectionMode(bool toggle, string defaultFilter)
    {
      Instance.FolderSelectMode = toggle;
      Instance.Show(Instance.CurrentPath);
      SetDefaultFilter(defaultFilter);
    }

    public void OnCancelButtonClickedCustom()
    {
      Result = null;
      onCancel();
    }

    public void OnSubmitButtonClickedCustom()
    {
      string path = m_currentPath;
      if (filenameInputField.text.Length > 0)
        path = Path.Combine(path, filenameInputField.text);
      ...
      {
        if (!m_folderSelectMode)
          OnOperationSuccessfulCustom(path);
        else
          filenameImage.color = wrongFilenameColor;
      }
      else if (Directory.Exists(path))
    ...
    private void OnOperationSuccessfulCustom(string path)
    {
      Result = path;
      onSuccess(path);
    }
